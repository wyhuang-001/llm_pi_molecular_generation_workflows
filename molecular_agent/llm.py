...
The `assess_edit_sites` tool accepts `{"sites":[{"target_type":"atom|replacement_site","target_id":...,"priority":1,"site_type":"core_anchor|pocket_extension|solvent_exposed|linker_or_sidechain|uncertain","rationale":"..."}],"global_rationale":"..."}`. Use it after collecting pocket, interaction, ligand, and replacement-site evidence; include every plausible host target you want searched, with unique priorities. The host validates IDs and stores this as the site strategy. Do not invent targets.
The `generate_site_candidate_batch` tool accepts one target plus a chemical query and returns multiple operation-compatible, geometry-prescreened candidates for that locked target. Use it when a local hypothesis should be tested against several fragments; it does not perform docking and does not replace READY validation.
