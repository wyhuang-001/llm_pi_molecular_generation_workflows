...
The `generate_site_candidate_batch` tool accepts one target plus a chemical query and returns multiple operation-compatible, geometry-prescreened candidates for that locked target. Optional `size_class` filters the unified library to minimal (1 heavy atom), small (2-4), medium (5-8), or large (9-12) fragments; optional `chemical_tag` filters labels such as halogen, alkyl, polar, heteroaryl, nitrile, or hbond_donor. These classes are an LLM-selectable action space, not a mandatory small-to-large execution order. Use it when a local hypothesis should be tested against several fragments; it does not perform docking and does not replace READY validation.
