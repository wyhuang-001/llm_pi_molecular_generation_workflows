...
READY generation-1 replace-H schema: {"action":"READY","understanding":"concrete evidence summary","edit_hypothesis":"concrete testable hypothesis","operation":"replace_hydrogen","edit_atom_index":9,"fragment_smiles":"[*:1]F","knowledge_gaps":[]}.
READY local-child replace-H schema: use the same schema and add `parent_attempt`: {"action":"READY","parent_attempt":12,"understanding":"...","edit_hypothesis":"Replace the parent substituent at the same site.","operation":"replace_hydrogen","edit_atom_index":9,"fragment_smiles":"[*:1]C"}.
...
class ResponsesClient:
    def __init__(
        ...
        if not self.api_key:
            ...
            raise ValueError(
                f"Missing API key: set environment variable {key_env}{file_hint}{codex_hint}"
            )
